...
namespace SkiaExperiment.Tests;

[TestClass]
public partial class ExampleSkiaTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(Skia).Assembly;
        var type = assembly.GetType(typeof(Skia).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find Skia type.");
        Assert.AreEqual(typeof(Skia), type, "Type of Skia does not match expected type.");
    }
    ...
    [LabsUITestMethod]
    public void SimpleUIExamplePageTest(ExampleSkiaTestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<Skia_ClassicBinding>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("SkiaControl");
    ...
    [LabsUITestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleSkiaTestPage page)
    {
    ...
    public async Task ComplexAsyncUIExampleTest()
    {
        await EnqueueAsync(() =>
        {
            var component = new Skia_ClassicBinding();
            Assert.IsNotNull(component);
        });
    }
